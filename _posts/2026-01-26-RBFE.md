...
```mermaid
flowchart TB
  subgraph W["λ windows (run MD in each window)"]
    w0["λ0"] --> w1["λ1"] --> w2["λ2"] --> w3["..."] --> wN["λN"]
  end

  subgraph S["Samples collected"]
    s0["x ~ p(x|λ0)"] 
    s1["x ~ p(x|λ1)"] 
    s2["x ~ p(x|λ2)"] 
    sN["x ~ p(x|λN)"]
  ...
  subgraph E["Cross-evaluate energies (overlap)"]
    e0["Evaluate U(x|λ0), U(x|λ1)"]
    e1["Evaluate U(x|λ1), U(x|λ0), U(x|λ2)"]
    e2["Evaluate U(x|λ2), U(x|λ1), U(x|λ3)"]
    eN["Evaluate U(x|λN), U(x|λN-1)"]
  end
  ...
  subgraph M["Estimator"]
    bar["BAR (two-state)"]
    mbar["MBAR (multistate)"]
  end

  E --> bar
  E --> mbar

  mbar --> out["Output: G(λ0)...G(λN)<br/>ΔG = G(λN) − G(λ0)"]
  bar --> out
```